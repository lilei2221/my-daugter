...
public class MainActivity5 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.shiyan3d);

        listView = findViewById(R.id.listView);
        listView.setChoiceMode(ListView.CHOICE_MODE_MULTIPLE); // 启用多选模式

        // 定义示例数据
        String[] items = {"user1", "user2", "user3", "user4", "user5"};

        // 创建 ArrayAdapter 并绑定到 ListView
        adapter = new ArrayAdapter<>(this, R.layout.list_item2, R.id.text, items);
    ...
    private final ActionMode.Callback actionModeCallback = new ActionMode.Callback() {
        ...
        public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
            if (item.getItemId() == R.id.menu_delete) {
                ...
                Toast.makeText(MainActivity5.this, "删除了 " + selectedItems.size() + " 项", Toast.LENGTH_SHORT).show();
